...
class RAGService:
    ...
    def _initialize_rag_demo(self):
        """初始化RAG演示系统"""
        try:
            self.logger.info("初始化RAG系统...")
            self.rag_demo = RAGDemo(
            ...
            doc_count = self.rag_demo.load_documents()
            
            if doc_count > 0:
                self.logger.info(f"成功加载 {doc_count} 个文档")
                
                # 显示统计信息
                stats = self.rag_demo.get_stats()
            ...
            self.logger.error(f"初始化RAG系统失败: {e}")
            return False
    ...
    def start(self):
        ...
        try:
            ...
            # 初始化RAG系统
            if not self._initialize_rag_demo():
                self.logger.error("RAG系统初始化失败，退出服务")
                return 1
            
            # 启动文件监控
            self.rag_demo.start_monitoring()
            
            self.logger.info("RAG服务启动成功，开始监控文件变化...")
            ...
            while self.running:
                try:
                    ...
                    # 检查监控是否还在运行
                    if not self.rag_demo.is_monitoring():
...
def load_config(config_file: str = None) -> dict:
    ...
    script_dir = os.path.dirname(os.path.abspath(__file__))
    config_paths.extend([
        os.path.join(script_dir, "rag_config.json"),
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
示例用法:
  # 使用默认配置文件（./rag_config.json）
  python3 rag_service.py
  
  # 指定配置文件
  python3 rag_service.py --config /path/to/config.json
  
  # 命令行参数覆盖配置文件
  python3 rag_service.py --watch-dir ./my-docs --model-name bge-m3:latest
  
  # 使用相对路径（基于脚本目录）
  python3 rag_service.py --watch-dir ../documents
  
  # 使用用户目录
  python3 rag_service.py --watch-dir ~/Documents/knowledge-base
        """
    )
